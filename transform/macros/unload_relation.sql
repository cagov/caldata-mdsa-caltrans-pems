{% macro unload_relation(strip_leading_words=1, unload_partitioning=none) %}
  ...
  {% set key = key ~ ('' if partitioning else '.csv') %}  {# Change file extension to .csv #}
  ...
  {% set url = stage ~ '/' ~ path %}
      ...
      {% endif %}
      file_format = (type=csv, field_delimiter=',', skip_header=1)  {# Set file format to CSV with a delimiter and skip header #}
      {% if partitioning %}
