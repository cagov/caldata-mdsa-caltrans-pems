...
nearest_stations_within_buffer_miles as (
    select
        a.id,
        b.id as other_id,
        a.district,
    ...
    where a.type in ('HV', 'ML') and abs(delta_postmile) <= 40.0 and a.id != b.id
)

select * from nearest_stations_within_buffer_miles
