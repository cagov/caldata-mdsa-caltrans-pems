...
with station_status as (
    ...
    -- consider previous whole week data
    where
        sample_date between dateadd(day, -7 - (date_part('dow', current_date) + 6) % 7, current_date)
        and dateadd(day, -1 - date_part('dow', current_date), current_date)
),
...
station_counts as (
    ...
    -- consider the previous full week Monday to sunday data
    where
        sample_date between dateadd(day, -7 - (date_part('dow', current_date) + 6) % 7, current_date)
        and dateadd(day, -1 - date_part('dow', current_date), current_date)
),
...
station_counts_regression_model as (
    ...
    group by id, lane, other_lane, other_id, district, sample_week, sample_year
    having
