...
resource "aws_iam_user" "airflow_s3_writer" {
  name = "${var.caltrans_naming ? "custom_" : ""}${var.prefix}${local.iam_user_infix}-airflow-s3-writer"
}
...
resource "aws_iam_role" "snowflake_storage_integration" {
  name = "${var.caltrans_naming ? "custom_" : ""}${var.prefix}${local.iam_role_infix}-snowflake-storage-integration"
