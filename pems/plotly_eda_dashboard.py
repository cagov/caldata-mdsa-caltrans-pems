...
station_metadata = ibis_con.table(database = "TRANSFORM_DEV", schema = "DBT_MMARKS", name = "TBL_MOST_RECENT_STATION_META")
...
pd_station_agg_stats_by_day['FLOW_4_Pct_Expected_Obs'] = pd_station_agg_stats_by_day['FLOW_4_COUNT']/expected_num_daily_observations
pd_station_agg_stats_by_day['OCCUPANCY_1_Pct_Expected_Obs'] = pd_station_agg_stats_by_day['OCCUPANCY_1_COUNT']/expected_num_daily_observations
pd_station_agg_stats_by_day['SPEED_1_Pct_Expected_Obs'] = pd_station_agg_stats_by_day['SPEED_1_COUNT']/expected_num_daily_observations


# Convert SAMPLE_DATE to datetime
pd_station_agg_stats_by_day['SAMPLE_DATE'] = pd.to_datetime(pd_station_agg_stats_by_day['SAMPLE_DATE'])

print(pd_station_agg_stats_by_day.dtypes)

float16_colnames = ['FLOW_1_AVG','FLOW_2_AVG', 'FLOW_3_AVG', 'FLOW_4_AVG','OCCUPANCY_1_AVG', 'SPEED_1_AVG',  'FLOW_1_Pct_Expected_Obs', 'FLOW_2_Pct_Expected_Obs', 'FLOW_3_Pct_Expected_Obs', 'FLOW_4_Pct_Expected_Obs', 'OCCUPANCY_1_Pct_Expected_Obs', 'SPEED_1_Pct_Expected_Obs']
for col in float16_colnames:
    pd_station_agg_stats_by_day[col] = pd_station_agg_stats_by_day[col].astype('float16')
    

int16_colnames = ['FLOW_1_COUNT','FLOW_2_COUNT', 'FLOW_3_COUNT', 'FLOW_4_COUNT','OCCUPANCY_1_COUNT', 'SPEED_1_COUNT']
...
def create_number_tile(number, description,  p_txt_size = 19, h_txt_size = 45):
    ...
    """
    tile_layout = html.Div(
        dbc.Card(
            dbc.CardBody([
                html.H3(number, style={'textAlign': 'center', 'fontSize': h_txt_size
                                       , 'marginLeft': '10px','marginRight': '10px','marginTop': '6px','marginBottom': '6px'
                                       }),
                html.P(description, style={'textAlign': 'center', 'fontSize': p_txt_size
                                           , 'marginLeft': '10px','marginRight': '10px','marginTop': '3px','marginBottom': '6px'})
            ]),
            style={'border': '1px solid lightgrey', 'borderRadius': '5px', 'padding': '1px', 'textAlign': 'center', 'height': '94px'
                   , 'marginLeft': '10px','marginRight': '10px','marginTop': '0px','marginBottom': '0px'}
        ),
        style={}
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        ], style={'margin': '10px', 'display': 'flex', 'flex-direction': 'column', 'justify-content': 'flex-end'}),  # Adjust margin as needed

        html.Div([
            html.H3("Metric"),
            dcc.Dropdown(['FLOW_1_COUNT',
                          'FLOW_1_AVG',
                          ...
                          'SPEED_1_COUNT',
                          'SPEED_1_AVG',
                          'SPEED_1_Pct_Expected_Obs'], 'FLOW_1_AVG', id='plot-value'),
        ], style={'margin': '10px', 'width':'200px', 'display': 'flex', 'flex-direction': 'column', 'justify-content': 'flex-end'}),

        html.Div([
            dcc.Dropdown(
                id='null-toggle',
                options=[
                    {'label': 'Show Only Null Values', 'value': 'SHOW_NULL'},
                    {'label': 'Exclude Null Values', 'value': 'EXCLUDE_NULL'},
        ...
        html.Div([
            html.Div([
                ...
                dcc.Input(id='min_value_input', type='number', value=0, style={'marginRight': '10px', 'width': '60px'}),
            ], style={'display': 'inline-block', 'width': '120px'}),

            html.Div([
                html.Label('Max Map Value:'),
                dcc.Input(id='max_value_input', type='number', value=100, style={'width': '60px'}),
            ], style={'display': 'inline-block', 'width': '120px'})
        ...
        ], style={'marginLeft': '60px','marginRight': '5px','marginTop': '0px','marginBottom': '0px'
                  , 'display': 'flex', 'justify-content': 'flex-end', 'height': '100px'}),

    ], style={'display': 'flex', 'justify-content': 'start', 'align-items': 'flex-end'}),  # This line makes the layout horizontal
    html.Div([
    ...
    html.Div([
        ...
        dcc.Input(id="color_range_max_percentile", type="number", value=0.95, size = "3")
    ], style={'margin': '10px','width':'200px', 'display': 'flex', 'flex-direction': 'column', 'justify-content': 'flex-end', "marginTop": "200px"}),
    html.Div([
        html.Label('Min Color:'),
        daq.ColorPicker(id='min-color-picker', value={'hex': '#FFC639'}, size = 140),
        
        html.Label('Max Color:'),
        daq.ColorPicker(id='max-color-picker', value={'hex': '#0079D4'}, size = 140)#,
        
        # html.Label('Null Color:'),
        # daq.ColorPicker(id='null-color-picker', value={'hex': '#888888'}),
    ], style={'margin': '10px','display': 'flex', 'justify-content': 'start', 'align-items': 'flex-end'})
])


# Callback for updating the graph based on selected date range
@app.callback(
...
def update_graph(start_date, end_date, agg_col, color_range_max_percentile,min_color, max_color, null_toggle, min_value, max_value):
    ...
    aggregated_df = filtered_df.groupby('ID').agg({
        agg_col: 'mean' }).reset_index()


    aggregated_df_filtered = aggregated_df[((aggregated_df[agg_col] >= min_value) & 
    ...
    if null_toggle == 'SHOW_NULL':
        pd_station_metadata_w_agg_results = pd_station_metadata_w_agg_results[pd_station_metadata_w_agg_results[agg_col].isnull()]
    elif null_toggle == 'EXCLUDE_NULL':
        pd_station_metadata_w_agg_results = pd_station_metadata_w_agg_results[pd_station_metadata_w_agg_results[agg_col].notnull()]
    ...
    fig = px.scatter_mapbox(
        ...
        lon="LONGITUDE",
        hover_name="NAME",
        hover_data={"FWY":True, "DIR":True, "ID":True, agg_col:':.2f', "TYPE":True, "LANES":True},
        custom_data  = "ID",
        color=agg_col,
        zoom=6,
        height=900,
    ...
    print(f'plot update complete')

    return fig

@app.callback(
    [Output('min_value_input', 'value'),
     Output('max_value_input', 'value')],
    [Input('date-picker-range', 'start_date'),
     Input('date-picker-range', 'end_date'),
     Input('plot-value', 'value')]#,
    # prevent_initial_call=True  # Prevents the callback from firing upon initial load
)
def update_default_min_max(start_date, end_date, agg_col):
...
def update_time_series(selectedData, agg_col,start_date,end_date):
    ...
    selected_ids = [point['customdata'][0] for point in selectedData['points']]  # Adjust according to your data structure
    print(selected_ids)
    ...
    filtered_data['ID'] = pd.Categorical(filtered_data['ID'], categories=selected_ids, ordered=True)

    # Sort the DataFrame by the 'ID' column
    filtered_data_sorted = filtered_data.sort_values(by='ID',ascending=False)
    SMALL_VALUE = 0.1
        
    filtered_data_sorted['Display_Value'] = filtered_data_sorted[agg_col].apply(lambda x: SMALL_VALUE if x == 0 else x)
    filtered_data_sorted['Day_of_Week'] = filtered_data_sorted['SAMPLE_DATE'].dt.day_name()
    ...
    fig = px.bar(filtered_data_sorted, x='SAMPLE_DATE', y='Display_Value', facet_col='ID'
                 ,facet_col_wrap=1
                  ,height=700
                  ,width=600
    ...
    fig.update_yaxes(range=[0, max([filtered_data_sorted[agg_col].max(), 5])], title_text='')
    ...
    for i, ID in enumerate(selected_ids, start=1):
        ...
        dir = id_metadata_row['DIR']
        title_str = name + " ("+fwy + " Fwy, " + dir +", ID = " + str(ID)+ ")"
...
@app.callback(
    ...
    Input('plot-value', 'value'), Input('overall_map', 'selectedData')]
    # Input('date-picker-range', 'start_date'),
    # Input('date-picker-range', 'end_date'),
)
def update_indiv_obs_time_series(click_data, agg_col, map_selected_data):
    if click_data is None or len(click_data['points']) == 0 or map_selected_data is None or len(map_selected_data['points']) == 0:
        # Display a message
        return {
            'data': [],
            'layout': {
    ...
    # x_key = next((key for key in selectedData['range'] if key.startswith('x')), None)
    # timestamp_strings = selectedData['range'][x_key]
    # date_strings = [ts.split(' ')[0] for ts in timestamp_strings]
    selected_ids = [str(point['customdata'][0]) for point in map_selected_data['points']] 
...
#
    ...
    SMALL_VALUE = 0.1
    result_sorted['Display_Value'] = result_sorted[agg_col.upper()].apply(lambda x: SMALL_VALUE if x == 0 else x)
    
    
    fig = px.bar(result_sorted, x='ROUNDED_TIMESTAMP', y=agg_col.upper(), facet_col='ID'
                 ,facet_col_wrap=1
                  ,height=700
                  ,width=600
                  ,template = 'plotly_white'
...
def update_tiles(agg_col, start_date, end_date,null_toggle):
    ...
    ttl_obs = format_number_for_dashboard(filtered_df[selected_field_count_column].sum())
    ...
    tile3_layout = create_number_tile(f"{null_percentage:.1f}%", "Pct Sensors w/ No Data")
